...
export class ProductListComponent implements OnInit {
  ...
  public displayedColumns: string[] = ['category', 'name',  'description', 'size', 'type', 'price', 'image', 'actions'];
  ...
  loadProducts(): void {
    this.productService.getAll().subscribe(data => {
      this.dataSource = data as IProductResponse[];
    })
  }
  ...
  editProduct(product: IProductResponse): void {
    this.dialog.open(AddProductComponent, {
      ...
      if (result && result.prod) {
        this.productService.update(result.prod, product.id).then(() => {
          this.loadProducts();
